...
The durable boundary is a **small set of MAF-owned contracts around the autopilot**, kept narrow and explicit so it stays stable as vehicles and interfaces change. That boundary is also what makes staged autonomy practical: replay, recording, and model-serving can sit below mission and above raw sensors without collapsing the PX4-centered safety split into an opaque deployment graph. Phase 1 ships as two processes per vehicle — `maf_aerial` (mission BT + PX4 adapter + gateway link co-located) and `maf_recorder` standalone, so a runtime crash never takes the black box down with it. Explicit C++ loops own behavior, a small supervisor state machine gates mission authority, `maf_px4_adapter` remains the only module that writes PX4-facing offboard commands, and the vehicle plugs into [Tower](https://github.com/EthanMBoos/Tower) by publishing a protobuf extension payload to [tower-server](https://github.com/EthanMBoos/tower-server), creating a clean runtime seam for learning.

**Future work:** MAF is compatible with Isaac Sim because the authority split stays intact: PX4 remains active for flight control, estimation, and failsafes, while Isaac Sim trains and evaluates perception and mission-level policies above it; sensor ingest feeds perception state, policy or planner outputs feed BT, and BT still executes through `maf_px4_adapter`. In practice, Isaac Lab defines observation and action spaces at those interfaces and runs training with domain randomization and curriculum. Exported artifacts (ONNX, TensorRT, or similar) wire into the same BT node interfaces used by deterministic planners, so learned modules replace hand-coded ones in place — same goal-context input, command-stream output, and termination semantics from simulation to field, preserving replayability and crash isolation without weakening the PX4-centered safety model.
